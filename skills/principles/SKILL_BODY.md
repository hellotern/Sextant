...
**How GitNexus guidance is loaded:**

All dedicated sub-skills (`sextant:fix-bug`, `sextant:debug`, etc.) unconditionally include `tool-gitnexus/SKILL_BODY.md`. Its content is always present in context — no detection step is needed to load it.

**When to apply it:**

- **GitNexus MCP tools callable** → Apply the guidance in all steps marked 🔗. Prioritize MCP tool calls over manual reading and grep searches. If a tool returns an index-not-found error (e.g. `.gitnexus/` does not exist), treat GitNexus as unavailable and fall back to the manual workflow.
- **GitNexus MCP tools unavailable** → Ignore all 🔗 markers and follow the original workflow. No existing workflow is affected.

> **Fallback skill note:** When `sextant` (this file) is loaded directly — rather than via a dedicated sub-skill — `tool-gitnexus/SKILL_BODY.md` is NOT included. If the GitNexus MCP tools are callable, apply the GitNexus guidance manually for steps marked 🔗.
